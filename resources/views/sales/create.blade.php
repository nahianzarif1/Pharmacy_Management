<x-app-layout>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('sales.store') }}" id="sale-form">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Invoice No (optional)</label>
                            <input type="text" name="invoice_no" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" name="payment_method" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" placeholder="Cash/Card/UPI" />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        <div>
                            ...
                            <input type="number" step="0.01" min="0" name="discount" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" value="0" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Tax</label>
                            <input type="number" step="0.01" min="0" name="tax" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" value="0" />
                        </div>
                        <div></div>
                    </div>

                    <div class="flex items-center justify-end gap-3">
                        <a href="{{ route('sales.index') }}" class="px-4 py-2 border rounded">Cancel</a>
                        <button class="px-4 py-2 bg-blue-600 text-white rounded">Save Sale</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        const medicines = @json($medicines->map(fn($m)=>['id'=>$m->id,'label'=>$m->sku.' - '.$m->name,'price'=>$m->price_per_unit]));
        const itemRows = document.getElementById('item-rows');
        const addRowBtn = document.getElementById('add-row');

        function newRow(idx){
            const options = medicines.map(m=>`<option data-price="${m.price}" value="${m.id}">${m.label}</option>`).join('');
            return `<tr>
                <td class=\"px-4 py-2\">
                ...
                <td class=\"px-4 py-2\"><input type=\"number\" step=\"0.01\" min=\"0\" value=\"0\" name=\"items[${idx}][unit_price]\" class=\"w-full border rounded px-2 py-1\"/></td>
