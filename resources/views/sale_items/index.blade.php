...
<div class="container mx-auto py-6">
    <h1 class="text-2xl font-semibold mb-4">Sale Items</h1>

    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            @if($items->isEmpty())
                <div class="text-sm text-gray-500">No sale items found.</div>
            @else
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($items as $it)
                                <tr>
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ optional($it->sale)->invoice_no }}</td>
                                    ...
                                    <td class="px-6 py-4 text-sm">{{ $it->unit_price }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
